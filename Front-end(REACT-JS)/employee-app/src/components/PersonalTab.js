...
const PersonalTab = () => {
  ...
  const [dbAadharcopy, setdbAadharcopy] = useState(null);
  const [dbPassportCopy, setdbPassportCopy] = useState(null);
  const [dbDrivingLicenseCopy, setdbDrivingLicenseCopy] = useState(null);
  ...
  const handleReset = () => {
    ...
    setdbAadharcopy(null);
    setdbPassportCopy(null);
    setdbDrivingLicenseCopy(null);
    setdateOfBirth(null);
    ...
    //   licenseFile: null,
    // });
    // fileUpload.passportFile = null;
    // fileUpload.aadharFile = null;
    // fileUpload.licenseFile = null;
    setFileUpload({
      passportFile: "",
      aadharFile: "",
      licenseFile: "",
    });
    // (fileUpload.passportFile = ""),
    //   (fileUpload.aadharFile = ""),
  ...
  const handleView = (event) => {
    ...
    //     // setPersonalInfo({ ...personalInfo, dob: dbdob });
  ...
  const getPersonalDetails = () => {
    getPersonByUserId()
      .then((data) => {
        ...
        setdbAadharcopy(data.aadharCopy);
        setdbPassportCopy(data.passportCopy);
        setdbDrivingLicenseCopy(data.drivingLicenseCopy);
  ...
  const handleDelete = () => {
    ...
    alert(
      "On deleting personal details all other tabs data will be deleted automatically.Are you sure?"
    );
    // deletePersonalDetailsById()
    //   .then((data) => {
  ...
  const handleFileChange = (event, field) => {
    ...
    setFileUpload({ ...fileUpload, [field]: event.target.files[0] });
  ...
  useEffect(() => {
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      if (files.length == 3 && buttonPressed != "Update") {
        // let formData = new FormData();
        ...
        // call API here to create personal details
        if (buttonPressed === "Create") {
          createPersonalDetails(formData)
            .then((data) => {
              toast.success(
                "Personal Details Created Successfully!" + data.pid
              );
              handleReset();
              files.length = 0;
      ...
      let pid = localStorage.getItem("PersonalDetailId");

      if (pid) {
        if (Object.keys(formErrors).length === 0 && isSubmit) {
          personalInfo.passportCopy = "";
          personalInfo.aadharCopy = "";
          personalInfo.drivingLicenseCopy = "";

          let formData = new FormData();
          formData.append("jsonBodyData", JSON.stringify(personalInfo));
          // console.log("this is form data", formData.get("jsonBodyData"));
              ...
              <Card>
                ...
                <CardBody>
                  ...
                  <Form onSubmit={handleFormSubmit}>
                    ...
                    {/* Adding new code for passport,aadhar and licnese to show then vertically */}
                    {/* row seventh */}
                    ...
                    <Row>
                      <Col md={4}>
                        <FormGroup>
                          ...
                          {dbPassportCopy != null ? (
                            <img
                              src={`data:image/png;base64,${dbPassportCopy}`}
                              height={"200px"}
                              width={"344px"}
                              style={{ "border-radius": "30px" }}
                            />
                          ) : (
                            "No Image Selected/present"
                          )}
                      ...
                      <Col md={4}>
                        <FormGroup>
                          ...
                          {dbAadharcopy == null ? (
                            ...
                            <img
                              src={`data:image/png;base64,${dbAadharcopy}`}
                              height={"200px"}
                              width={"344px"}
                              style={{ "border-radius": "30px" }}
                            />
                      ...
                      <Col md={4}>
                        <FormGroup>
                          ...
                          />
                          <hr />
                          {dbDrivingLicenseCopy != null ? (
                            <img
                              src={`data:image/png;base64,${dbDrivingLicenseCopy}`}
                              height={"200px"}
                              width={"344px"}
                              style={{ "border-radius": "30px" }}
                            />
