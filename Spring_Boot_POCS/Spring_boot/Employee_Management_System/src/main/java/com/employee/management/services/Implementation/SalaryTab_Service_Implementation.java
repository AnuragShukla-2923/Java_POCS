...
public class SalaryTab_Service_Implementation implements SalaryTab_Service {
	...
	@Override
	public SalaryTabDto createSalary(SalaryTabDto Salary_tabDto,Long pid) {
		Salary_Tab salaryTab = this.modelMapper.map(Salary_tabDto, Salary_Tab.class);
		Personal_Tab personal_Tab = this.personalTabRepo.findById(pid).orElseThrow(()->new ResourceNotFoundException("Person", "PersonalId", pid));
		salaryTab.setPersonalId(personal_Tab);
